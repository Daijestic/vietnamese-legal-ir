...
class BooleanRetriever:
    ...
    def _and_search(self, tokens: list[str]) -> set[str]:
        ...
        for token in tokens[1:]:
            result &= self._postings_set(token)
        return result
    ...
    def _not_search(self, positive_tokens: list[str], negative_tokens: list[str]) -> set[str]:
        ...
        for token in negative_tokens:
            negative_result |= self._postings_set(token)
    ...
    def search(
        ...
        if re.search(r"\bNOT\b", raw_query, flags=re.IGNORECASE):
            left, right = re.split(r"\bNOT\b", raw_query, maxsplit=1, flags=re.IGNORECASE)
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Duong dan den inverted index",
    )
    ...
    if not index_path.exists():
        raise FileNotFoundError(
            f"Khong tim thay {index_path}. Hay chay: python -m src.indexing.inverted_index"
        )
